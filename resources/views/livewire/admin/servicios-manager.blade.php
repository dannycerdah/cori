<div class="rounded-[2rem] bg-white p-6 shadow-card dark:bg-slate-900 sm:p-8">
    ...
    <div class="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
        <div class="flex flex-col gap-2">
            ...
            <input id="buscar" type="text" wire:model.live.debounce.300ms="buscar" class="cori-input" placeholder="Nombre del servicio">
        </div>
        <div class="flex flex-col gap-2">
            <label class="cori-label" for="filtroEspecialidad">Especialidad</label>
            <select id="filtroEspecialidad" wire:model.live="filtroEspecialidad" class="cori-input">
        ...
        <table class="min-w-full divide-y divide-slate-100 dark:divide-white/10">
            <thead class="bg-gradient-to-r from-brand-blue to-brand-pink text-white">
                <tr>
                    ...
                    <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider">Requiere doctor</th>
            ...
            <tbody class="divide-y divide-slate-100 bg-white dark:divide-white/10 dark:bg-slate-900">
                @forelse ($servicios as $servicio)
                    <tr class="transition hover:bg-brand-soft/50 dark:hover:bg-white/5">
                        <td class="px-6 py-4 text-sm font-semibold text-brand-blue dark:text-white">{{ $servicio->nombre }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{{ $servicio->especialidad?->nombre }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{{ $servicio->requiere_doctor ? 'Sí' : 'No' }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-sm">
                            <button wire:click="toggleEstado({{ $servicio->id }})"
                                class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold transition
                                    {{ $servicio->estado ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200 dark:bg-emerald-400/10 dark:text-emerald-300' : 'bg-slate-200 text-slate-600 hover:bg-slate-300 dark:bg-white/10 dark:text-slate-300' }}">
                                {{ $servicio->estado ? 'Activo' : 'Inactivo' }}
                            </button>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                            <div class="flex justify-end gap-1.5">
                                <button wire:click="editar({{ $servicio->id }})" class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-white/10 dark:bg-transparent dark:text-slate-300">Editar</button>
                                <button wire:click="eliminar({{ $servicio->id }})" wire:confirm="¿Eliminar este servicio?" class="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700 transition hover:bg-rose-100 dark:border-rose-400/20 dark:bg-rose-400/10 dark:text-rose-300">Eliminar</button>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-400 dark:text-slate-500">No hay servicios registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4"
            ...
            <div class="relative w-full max-w-lg rounded-[2rem] bg-white p-6 shadow-card dark:bg-slate-900 sm:p-8">
                ...
                <form wire:submit="guardar" class="mt-6 space-y-5">
                    <div>
                        <label class="cori-label" for="especialidad_id">Especialidad</label>
                        <select id="especialidad_id" wire:model="especialidad_id" class="cori-input">
                    ...
                    <div>
                        ...
                        <input id="nombre" type="text" wire:model="nombre" class="cori-input" placeholder="Ej. Consulta cardiológica">
                    ...
                    <div>
                        ...
                        <textarea id="descripcion" wire:model="descripcion" rows="3" class="cori-input resize-none"></textarea>
                    ...
                    <label class="flex items-center gap-3 text-sm font-medium text-slate-700 dark:text-slate-200">
                        <input type="checkbox" wire:model="requiere_doctor" class="h-4 w-4 rounded border-slate-300 text-brand-pink focus:ring-brand-pink">
                        Requiere doctor asignado
                    </label>
